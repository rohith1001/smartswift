...
export class HolidaysComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.holidaysService.query().subscribe(
            (res: HttpResponse<IHolidays[]>) => {
    ...
    trackId(index: number, item: IHolidays) {
        return item.id;
    }

    registerChangeInHolidays() {
        this.eventSubscriber = this.eventManager.subscribe('holidaysListModification', response => this.loadAll());
    }
